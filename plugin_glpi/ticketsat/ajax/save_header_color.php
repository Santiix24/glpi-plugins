...
if (!empty($_POST['reset'])) {
    ...
    $DB->delete('glpi_configs', ['context'=>'ticketsat','name'=>'notification_header_color']);
    echo json_encode(['success'=>true]);
    exit;
}

// Guardar color de la notificación (color sólido único)
if (!empty($_POST['notification_color'])) {
    $nc = trim($_POST['notification_color']);
    if (!preg_match('/^#[0-9a-fA-F]{6}$/', $nc)) {
        echo json_encode(['success'=>false,'message'=>'Color inválido']);
        exit;
    }
    ts_save_cfg('notification_header_color', $nc);
    echo json_encode(['success'=>true]);
    exit;
}
