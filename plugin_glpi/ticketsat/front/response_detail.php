    ...
    <div class="card mb-2">
        <div class="card-body">
            <p class="mb-1"><strong><?= htmlspecialchars($q['question']) ?></strong>
               <span class="badge text-bg-primary ms-2" style="font-weight:600"><?= PluginTicketsatQuestion::getTypeLabel($q['type']) ?></span></p>
            <?php if (!$ans): ?>
            ...
            <?php elseif (in_array($q['type'], ['scale', 'stars'])): ?>
                ...
                <span class="badge text-bg-primary fs-6"><?= $val ?> / <?= $max ?></span>
                <?php if ($q['type'] === 'stars'): ?>
                <?php for ($i = 1; $i <= 5; $i++): ?>
