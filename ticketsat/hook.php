...
function plugin_ticketsat_display_central() {
    ...
    echo '<div class="card-header d-flex align-items-center gap-2" style="background:#6C63FF;color:#fff;font-weight:700;font-size:.93rem">';
    ...
    foreach ($pending as $r) {
        $color      = htmlspecialchars($r['header_color'] ?? '#6C63FF');
        ...
        echo '<div style="width:10px;height:10px;border-radius:50%;background:' . $color . ';flex-shrink:0"></div>';
        ...
        echo '</div>';
        echo '<span style="font-size:.78rem;color:#fff;background:' . $color . ';padding:4px 12px;border-radius:20px;font-weight:700;flex-shrink:0">Responder</span>';
        echo '</a>';
    }
    echo '</div></div>';
}
